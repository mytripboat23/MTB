...
adminSecure();
$table_caption = "Set Google Auth";
if($_POST['SetGoogleAuth'])
{
		$arrAE['set_google_auth'] = $obj->filter_mysql($_POST['set_google_auth']);
		$obj->updateData(TABLE_SETTINGS,$arrAE,"set_id='1'");
		$obj->add_message("message","Google Auth Setting ".UPDATE_SUCCESSFULL);
              ...
              <div class="col-xs-12">
                <form class="form-horizontal" role="form" name="googleAuth" id="googleAuth" method="post" action="">
				   ...
				   <div class="form-group">
                    <label class="col-md-4 control-label no-padding-right" for="form-field-1"> Google Auth Required:</label>
                    <div class="col-md-4">
                      	<select name="set_google_auth"  class="nav-search-input" id="set_google_auth" >
							<option value="n" <?php if($site_set['set_google_auth']=='n'){?> selected="selected"<?php }?>>No</option>
							<option value="y" <?php if($site_set['set_google_auth']=='y'){?> selected="selected"<?php }?>>Yes</option>
						</select>
                    </div>
                  </div>
