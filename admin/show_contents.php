...
$sql=$obj->selectData(TABLE_CONTENT,"","content_status<>'Deleted' ".$extra." order by content_id asc",2);
      ...
      <tbody>
	   ...
	   ?>
        <tr>         
          ...
          <td><?=$obj->short_description_cw($data['content_descr'],100);?></td>
          <td><?=date("M d, Y",strtotime($data['content_modified']));?></td>
          <td><?=$data['content_status'];?></td>
          <td>
		   <a href="edit_content.php?cId=<?=$data['content_id']?>&return_url=<?=urlencode($_SERVER['REQUEST_URI'])?>" class="tooltip-info" data-rel="tooltip" title="Edit">Edit</a>		  		  </td>
