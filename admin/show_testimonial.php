...
if(isset($_POST['btnDelete']))
{
	if(is_array($_POST['chkSelect']))
	{
		foreach($_POST['chkSelect'] as $chbxAr)
		{
			...
			$deleteStatus = $obj->updateData(TABLE_TESTIMONIAL,array('test_status'=>'Deleted'),"test_id='".$chbx."'"); 
		}
...
$sql=$obj->selectData(TABLE_TESTIMONIAL,"","test_status<>'Deleted'".$extra." order by test_id desc",2);
    ...
    <div class="page-content">
      ...
      <div class="page-content-area">
        <div class="page-header">
          <h1> Dashboard <small> <i class="ace-icon fa fa-angle-double-right"></i> Testimonial Management <span class="pull-right"><a href="edit_testimonial.php?return_url=<?=urlencode($_SERVER['REQUEST_URI'])?>"><i class="fa fa-plus-square"></i> Add Testimonial</a></span></small> </h1>
        </div>
        <!-- /.page-header -->
      ...
      <tbody>
	   ...
	   ?>
        <tr>         
          ...
          <td><?=substr(strip_tags(html_entity_decode($data['test_desc'])),0,100);?>...</td>
		  <td><img src="<?='../'.TESTIMONIAL.$data['test_image'];?>" height="50" width="50"></td>
          <td><?=date("M d, Y",strtotime($data['test_modified']));?></td>
          <td><?=$data['test_status'];?></td>
          <td><a href="edit_testimonial.php?cId=<?=$data['test_id']?>&return_url=<?=urlencode($_SERVER['REQUEST_URI'])?>" class="tooltip-info" data-rel="tooltip" title="Edit">Edit</a>		  		  </td>
		   <td class="center"><label class="position-relative">
              <input type="checkbox" class="ace" name="chkSelect[]" id="chkSelect1" value="<?=$data['test_id'];?>"/>
