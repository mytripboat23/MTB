...
if($_REQUEST['action']=='yes' && $_REQUEST['oId']!='')
{
	...
	$packD = $obj->selectData(TABLE_LOGIN_AD_STAKER_PACKAGE,"","order_id='".$orderID."'",1);
	$lasp_id = $packD['lasp_id'];
	...
	$lapArr['order_id']    		= 0;	
	$lapArr['user_id']    		= 0;
	$lapArr['lasp_pstatus'] 		= 'u';
	$lapArr['lasp_assign'] 		= 'a';
	$lapArr['lasp_u_admin_id'] 	= $_SESSION[ADMIN_SESSION_NAME]['admin_id'];	 
	$lapArr['lasp_pstatus_date'] = '0000-00-00 00:00:00';	 
	$obj->updateData(TABLE_LOGIN_AD_STAKER_PACKAGE,$lapArr,"lasp_id='".$lasp_id."'"); 
...
//echo $sql = $obj->selectData(TABLE_ORDER." as o, ".TABLE_ORDER_DETAILS." as od, ".TABLE_LOGIN_AD_STAKER_PACKAGE." as lp","o.*,od.*,lp.lasp_date","o.order_id=od.order_id and o.order_pstatus='p' and o.order_status='Active' and od.lasp_id !='0' and lp.lasp_id=od.lasp_id and lp.lasp_pstatus='p' and lp.lasp_assign='m' and od.od_status='Active' order by od.lasp_id desc",2);

$sql = $obj->selectData(TABLE_LOGIN_AD_STAKER_PACKAGE,"","lasp_pstatus='p' and lasp_assign='r' and lasp_status='Active'".$extra." order by lasp_id desc",2);
  ...
  <div class="main-content">
    <div class="breadcrumbs" id="breadcrumbs">
      ...
      <ul class="breadcrumb">
        ...
        <li class="active">Login Ad Stakers Manual Assing</li>
      </ul>
    </div>
    <div class="page-content">
      <div class="page-content-area">
        <div class="page-header">
          <h1> Dashboard <small> <i class="ace-icon fa fa-angle-double-right"></i> Login Ad Stakers Manual Assing Management <span class="pull-right"><a href="edit_manual_assign_login_ad_stk.php?return_url=<?=urlencode($_SERVER['REQUEST_URI'])?>"><i class="fa fa-plus-square"></i> Add Login Ad Stakers Manual Assing</a> </span> </small> </h1>
        </div>
        <!-- /.page-header -->
        <div class="row">
          <div class="col-xs-12">
	   ...
	   ?>
                      <tr>
                        <td>#
                          <?=10000+$data['order_id'];?></td>
                        <td><?=$userD['user_first_name']." ".$userD['user_last_name'];?>
                          <br>
                          (
                          <?=$userD['user_email']?>
                          )</td>
                        <td><?=$data['lasp_date'];?>
                          - Login Ad</td>
                        <td><?=$orderD['order_tot_asimi'];?>
                          Asimi</td>
                        <td><?=$orderD['order_created'];?></td>
                        <td><?php 
			$CurDate = date('Y-m-d');
			if($data['lasp_date'] > $CurDate) {
			?>
...
{
	if(confirm("Are you sure you want to Remove Login Ad Staker?"))
	{	
		window.location.href = 'show_manual_assign_login_ad_stk.php?oId='+oId+'&action=yes&return_url='+rUrl;
	}
}
</script>
</body>
