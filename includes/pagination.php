	...
	function pagewise($numrows,$pageno=1,$lnkParam="",$limit="",$lnkScr="")
	{
		...
		if($selectedPg!=1)
		{
			...
			if($this->nextprev==1)
			{
				...
				{
				$club='&club_id='.$_REQUEST['club_id'];
				}
				$newpageno = abs($selectedPg - 1);
				$pgStr.="<a class='".nextprevious."' href=\"".$this->linksrc."?pageno=".$newpageno.$lnkParam.$club."\">".$this->prevText."</a>";
			}
		}
		if(!$this->simplepaging && !$this->dropdownpaging)
		{
			for ($i=$st_page; $i<=$end_page; $i++)
			{
				if($selectedPg == $i)
				{
					$pgStr.="<span class='".page_nev_search_current."'>".$i."</span>";
				}
				else
				{
					if($_REQUEST['club_id'] > 0)
					...
					$pgStr.="<a class='".page_nev_search."' href=\"".$this->linksrc."?pageno=".$i.$lnkParam.$club."\">".$i."</a>";
				}
			}
		}
		...
		if($selectedPg!=$pages)
		{
			if($this->nextprev==1)
			{
				...
				$newpageno = abs($selectedPg+1);
				$pgStr.="<a class='".previousnext."' href=\"".$this->linksrc."?pageno=".$newpageno.$lnkParam.$club."\">".$this->nextText."</a>";
			}
			if(!$this->normalpaging && $this->firstlast==1)
			{
