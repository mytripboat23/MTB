...
function curPageURL() {
	...
	function currencySelect($selval)
	{	
		$selvals = explode(",",trim($selval,","));
		$res=parent::selectData(TABLE_CURRENCY,"","curr_status='A'","");
		while($row=mysqli_fetch_array($res))
		{
			$str.="<option value='".$row['curr_id']."'";
			if(in_array($row['curr_id'],$selvals,true))
			{
				$str.=' selected';
			}
			$str.=">".$row['curr_title']."</option>";
		}
		return $str;
	}
	
	function getCurrency($currId){
		$res = parent::selectData(TABLE_CURRENCY,"curr_title","curr_status='A' and curr_id='".$this->filter_mysql($currId)."'",1);
		return($res['curr_title']);
	}
