...
class pagingRecords extends db_connect 
{
	var $yahoopaging;
	var $googlepaging;
	var $normalpaging;
	...
	function queryError($sql="")
	{
		...
		if($this->showerror)
		{
			echo 'ERROR:'.mysqli_error($link).'<br>ERRNO:'.mysqli_errno($link).'<br>SQL:'.$sql.'<br>';
		}
		else
		{
			echo PAGE_PROBLEM;
	...
	function runQueryPaging($sql,$pageno="",$lnkParam="",$lnkScr="")
	{
		...
		$res=mysqli_query($link,$sql) or $this->queryError($sql);
		...
		if(!$this->error)
		{
			...
			if($numrows>0)
			{
				...
				$this->SQL=$sql;
				$res=mysqli_query($link,$sql) or $this->queryError();
				$numrowsl=@mysqli_num_rows($res);
	...
	function pagewise($numrows,$pageno=1,$lnkParam="",$limit="",$lnkScr="")
	{
		...
		if($selectedPg!=1)
		{
			...
			{	
				if($this->htpaging){
					$pgStr.="<a class='".CSSFL."' href=\"".FURL.$this->htpaging."/page/1/".$lnkParam."\">".$this->firstText."</a>&nbsp;  ";
				} else {
			...
			if($this->nextprev==1)
			{
				...
				if($this->htpaging){
					$pgStr.="<a class='".CSSNP."' href=\"".FURL.$this->htpaging."/page/".$newpageno."/".$lnkParam."\">".$this->prevText."</a>&nbsp;  ";
				} else {
		...
		if(!$this->simplepaging && !$this->dropdownpaging)
		{
			for ($i=$st_page; $i<=$end_page; $i++)
			{
				...
				{	
					if($this->htpaging){
						$pgStr.="<a class='".CSSPN."' href=\"".FURL.$this->htpaging."/page/".$i."/".$lnkParam."\">".$i."</a>&nbsp;&nbsp;";
					} else {
		...
		if($selectedPg!=$pages)
		{
			if($this->nextprev==1)
			{
				$newpageno = abs($selectedPg+1);
				if($this->htpaging){
					$pgStr.="&nbsp;<a class='".CSSNP."' href=\"".FURL.$this->htpaging."/page/".$newpageno."/".$lnkParam."\">".$this->nextText."</a>";
				} else {
			...
			{
				if($this->htpaging){
					$pgStr.=" &nbsp;<a class='".CSSFL."' href=\"".FURL.$this->htpaging."/page/".$pages."/".$lnkParam."\">".$this->lastText."</a>";
				} else {
