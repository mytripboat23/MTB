...
$searchTSql = $obj->selectData( TABLE_PACKAGE, "", "pck_status='Active' and (pck_start_dt > '" . date( "Y-m-d" ) . "' or ( pck_month >= '" . date( "n" ) . "' and pck_year = '" . date( "Y" ) . "') or ( pck_year = '" . date( "Y" ) . "'))  and pck_id < '" . $last_tour_id . "'", "", "pck_id desc", "", "0,5" );
...
<div class="single_list_tour">
    ...
    <div class="tour_upper">
      <div href="#" class="operater profile_listing"> <a href="#"><img src="<?=AVATAR.$uphoto;?>" width="60"></a>
        <div class="like_area"> <a id="pckl_<?=$searchTData->pck_id?>" href="javascript:void(0);" <?php if($obj->get_pck_like_status($searchTData->pck_id,$log_user_id)){?> class="active"<?php }?><?php if($searchTData->user_id!=$log_user_id){ ?>onclick="set_unset_pck_like('<?=$searchTData->pck_id?>','<?=$log_user_id?>')"<?php }?>> <span class="material-symbols-outlined"> favorite </span> </a>
          <div class="agent_pop">
            <h4>
              <?php if($userD['user_display_name']!=''){ echo $userD['user_display_name']; }else{ echo $userD['user_full_name'];;}?>
            </h4>
      ...
      <div class="tour_info">
        <div class="tour_about">
          <h4><a href="tour-details?tId=<?=$searchTData->pck_id?>">
        ...
        <div class="share_price_area">
          <div class="tour_price"> 
            ...
            <p><span class="badge rounded-pill text-bg-warning"><strong><?php echo $obj->getCurrency($searchTData->pck_curr);?></strong>
              <?=$searchTData->pck_price?>
              /-  </span>
            <?php if($searchTData->pck_foreign_price>0){?>
            
				<span class="badge rounded-pill text-bg-warning">
				<strong><?php echo $obj->getCurrency($searchTData->pck_foreign_curr);?></strong>
              <?=$searchTData->pck_foreign_price?>
              /-<small>For foreign tourist</small> </span>
            <?php }?>
          </div>
 ...
 <strong id="pck_count_<?=$searchTData->pck_id;?>">
              <?=$obj->get_pck_like_count($searchTData->pck_id);?>
              </strong></p>
          </div>
        </div>
      </div>
    ...
    <div class="tour_bottom">
      <p>
        <?=$obj->short_description_cw($searchTData->pck_desc,100);?>
      </p>
    </div>
  </div>
</div>
