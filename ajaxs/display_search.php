...
$searchTSql  = $obj->selectData(TABLE_PACKAGE,"","pck_dest like '%".$search_dest."%' and (pck_start_dt > '".$search_dt."' or ( pck_month >= '".$month."' and pck_year = '".$year."') or ( pck_year > '".date("Y")."')) and pck_end_dt>= '".date("Y-m-d")."' and pck_status='Active' and pck_id> '".$last_tour_id ."'","","pck_id asc","","0,5");

if(isset($_REQUEST['search_filter']) && $_REQUEST['search_filter']!='')
{
	if($_REQUEST['search_filter']=='price-high-low')
	{
		$searchTSql = $obj->selectData(TABLE_PACKAGE,"","pck_dest like '%".$search_dest."%' and (pck_start_dt > '".$search_dt."' or ( pck_month >= '".$month."' and pck_year = '".$year."') or ( pck_year > '".date("Y")."')) and pck_end_dt>= '".date("Y-m-d")."'  and pck_status='Active' and pck_id> '".$last_tour_id ."'","","pck_price desc","","0,5");
	}
	if($_REQUEST['search_filter']=='price-low-high')
	{
		$searchTSql = $obj->selectData(TABLE_PACKAGE,"","pck_dest like '%".$search_dest."%' and (pck_start_dt > '".$search_dt."' or ( pck_month >= '".$month."' and pck_year = '".$year."') or ( pck_year > '".date("Y")."')) and pck_end_dt>= '".date("Y-m-d")."'  and pck_status='Active' and pck_id> '".$last_tour_id ."'","","pck_price asc","","0,5");
	}
	if($_REQUEST['search_filter']=='top-agent')
	{
		...
		$searchTSql = $obj->selectData(TABLE_PACKAGE,"","pck_dest like '%".$search_dest."%' and (pck_start_dt > '".$search_dt."' or ( pck_month >= '".$month."' and pck_year = '".$year."') or ( pck_year > '".date("Y")."')) and pck_end_dt>= '".date("Y-m-d")."'  and pck_status='Active' and pck_id> '".$last_tour_id ."'","",$order_by,"","0,5");
	}
}
              ...
              <div class="tour_upper">
                ...
                <div class="tour_info">
                  <div class="tour_about">
                    <h4><a href="tour-details.php?tId=<?=$searchTData->pck_id?>"><?=$searchTData->pck_title?></a>
					  
					   <?php if($duration!=""){?>   <span class="badge rounded-pill text-bg-light"><?=$duration;?></span><?php }?>
                  ...
                  <div class="share_price_area">
                     <?php /*?> <div class="tour_price">
                        <p><span class="badge rounded-pill text-bg-warning"><strong><?php echo $obj->getCurrency($searchTData->pck_curr);?></strong> <?=$searchTData->pck_price?> /-  </span>
						 <?php if($searchTData->pck_foreign_price>0){?>
					     <span class="badge rounded-pill text-bg-primary"><strong><?php echo $obj->getCurrency($searchTData->pck_foreign_curr);?></strong> <?=$searchTData->pck_foreign_price?> /-<small>For foreign tourist</small></span></p>
					     <?php }?>
                      </div><?php */?>
